...
const isDebugLogEnabled = process.env.NODE_ENV !== 'production';
...
const transformDataForChart = (data: any[], visibleMaterials: string[], interval: 'weekly' | 'monthly' | 'yearly') => {
  ...
  debugLog('transformDataForChart 입력:', {
  ...
  debugLog('transformDataForChart 결과:', {
...
const MaterialsChart: React.FC<MaterialsChartProps> = ({ tableRowCount = 0 }) => {
  ...
  const chartData = useMemo(() => {
    ...
    if (!rawData) {
      debugLog('rawData가 undefined 또는 null입니다.');
      return [];
    }
  ...
  const gridHorizontalPoints = useMemo(() => {
    ...
    debugLog('gridHorizontalPoints (실제ticks기준):', middleTicks, 'allTicks:', leftAxisConfig.ticks);
    return middleTicks;
